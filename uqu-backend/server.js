...
console.log('[SERVER] Starting up...'); // <--- ADD THIS LINE
...
// Middleware
app.use(cors({
  origin: ['http://localhost:3000', 'http://localhost:5678, http://localhost:3001'],
  credentials: true
}));
app.use(bodyParser.json());

// Load UQU data
let UQU_DATA; // <--- Declare UQU_DATA here
try {
  console.log('[SERVER] Attempting to load uqu_unified_backend.json...'); // <--- ADD THIS LINE
  const jsonData = fs.readFileSync(path.join(__dirname, 'uqu_unified_backend.json'), 'utf8');
  UQU_DATA = JSON.parse(jsonData);
  console.log('[SERVER] Successfully loaded uqu_unified_backend.json.'); // <--- ADD THIS LINE
} catch (error) {
  console.error('[SERVER] CRITICAL ERROR loading uqu_unified_backend.json:', error); // <--- ADD THIS LINE
...
app.post('/api/chat-relay', async (req, res) => {
  ...
  const reply = await chatHandler(message, context);
  res.json(reply);
});
...
// Start server
console.log(`[SERVER] Attempting to listen on port ${PORT}...`); // <--- ADD THIS LINE
app.listen(PORT, () => {
